...
import { bluRayValidationSchema } from 'validationSchema/blu-rays';
import { CollectionInterface } from 'interfaces/collection';
import { BluRayInterface } from 'interfaces/blu-ray';

function BluRayCreatePage() {
  const router = useRouter();
  const [error, setError] = useState(null);
  const roqClient = useRoqClient();
  const handleSubmit = async (values: BluRayInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await roqClient.blu_ray.create({ data: values as RoqTypes.blu_ray });
      resetForm();
      router.push('/blu-rays');
    } catch (error) {
      setError(error);
    }
  };

  const formik = useFormik<BluRayInterface>({
    initialValues: {
      title: '',
      description: '',
    ...
    validationSchema: bluRayValidationSchema,
